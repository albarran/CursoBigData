...
* Notad que el nombre completo de un objeto de R es `paquete::nombre`
...
```{r eval=FALSE, echo=F}
# * Se accede a variables del marco de datos con '$'

```
```{r eval=FALSE, echo=T}
head(Affairs$gender)
head(gender)          # no se encuentra
```
...
* Ahora se accede a las variables solo por su nombre. 
...
* ¿Cómo obtendríaamos la moda?

```{r eval=FALSE, echo=F}
# tabla <- table(wage); tabla
# ma <- max(tabla); ma
# moda <- tabla[tabla==ma]; str(moda); moda
# dimnames(moda)
#
...
* en lugar de puntos, type = "p" (defecto), se puede usar líneas 'type="l"' 
...
ej. 'utils' en R base y 'readr' ('tidyverse') 
...
* Texto plano con delimitadores
     ...
     + Una línea por caso, columnas separadas por delimitador 
...
## Funciones en 'readr' (parte de 'tidyverse') 
* Ofrece versiones equivalentes a las de 'utils': read_csv, read_tsv y read_delim
...
* Cuidado con NAs y que las longuitudes de los vectores sean iguales
