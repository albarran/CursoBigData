...
* Es un algoritmo específico de agregación de árboles que introduce aleatorización para quitar correlación entre los árboles.

* Construimos un número de árboles de decisión en muestras de entrenamiento de bootstrap
...
* Este proceso mitiga la fuerza de regresores muy fuertes, permitiendo una mayor variación en los árboles embolsados
...
RailTrail_modelo_RF  <- rand_forest(mode = "regression",
                                       mtry = 3, trees = 100) %>% 
                          set_engine("ranger", #randomForest, spark
                                     importance = "impurity")   
